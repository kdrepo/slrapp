...
class MinerUStopRequested(RuntimeError):
    pass


def parse_lit_paper_pdf_to_markdown(lit_paper_id, stop_check=None, api_token=None):
    paper = LitPaper.objects.select_related('review').get(pk=lit_paper_id)

    if not paper.fulltext_retrieved:
        raise ValueError(f'LitPaper {paper.id} has no retrieved fulltext.')
    if not paper.pdf_path:
        raise ValueError(f'LitPaper {paper.id} has empty pdf_path.')
    ...
    headers = _mineru_json_headers(api_token=api_token)
    file_name = os.path.basename(absolute_pdf_path)

    if stop_check and stop_check():
        raise MinerUStopRequested('Stop requested before MinerU upload.')

    batch_url = f"{_mineru_base_url()}/file-urls/batch"
    payload = {
        'files': [{'name': file_name, 'data_id': f'lit_paper_{paper.id}'}],
    ...
    for _ in range(max_polls):
        if stop_check and stop_check():
            raise MinerUStopRequested('Stop requested during MinerU polling.')
    ...
    if stop_check and stop_check():
        raise MinerUStopRequested('Stop requested before MinerU ZIP download.')
    ...
    return MinerUParseResult(ok=True, status='done', error='', markdown_chars=len(markdown_content or ''), batch_id=batch_id)


def parse_lit_review_pdfs_with_mineru(review_id, retry_failed_only=False, progress_callback=None, stop_check=None):
    review = LitReview.objects.get(pk=review_id)

    papers_qs = review.papers.filter(fulltext_retrieved=True).exclude(pdf_path='').order_by('id')
    ...
    _emit(progress_callback, {'event': 'started', 'targeted': len(papers), 'paper_ids': [paper.id for paper in papers]})
    ...
    api_tokens = _mineru_api_tokens()
    max_workers = min(len(papers), len(api_tokens))
    ...
    try:
        while pending_index < len(papers) and len(future_to_paper) < max_workers:
            if stop_check and stop_check():
                stopped = True
                break
            paper = papers[pending_index]
            token = api_tokens[pending_index % len(api_tokens)]
            _emit(
                progress_callback,
                {
                    'event': 'processing',
            ...
            future = executor.submit(parse_lit_paper_pdf_to_markdown, paper.id, stop_check, token)
        ...
        while future_to_paper:
            if stop_check and stop_check():
                ...
                for pending_future in list(future_to_paper.keys()):
                    pending_future.cancel()
                future_to_paper.clear()
                break

            done_futures, _ = wait(future_to_paper.keys(), timeout=0.5, return_when=FIRST_COMPLETED)
            if not done_futures:
                continue
            ...
            while not stopped and pending_index < len(papers) and len(future_to_paper) < max_workers:
                if stop_check and stop_check():
                    stopped = True
                    break
                paper = papers[pending_index]
...
def clean_existing_lit_mineru_references(review_id, progress_callback=None, stop_check=None):
    review = LitReview.objects.get(pk=review_id)
    papers = list(
    ...
    for idx, paper in enumerate(papers, start=1):
        ...
        try:
            ...
            _emit(
                progress_callback,
                {'event': 'failed', 'paper_id': paper.id, 'title': paper.title, 'error_message': f'{exc.__class__.__name__}: {exc}'},
            )
...
def _strip_reference_section(markdown_text):
    ...
    atx_pattern = re.compile(
        r'^\s{0,3}#{1,6}\s*(references|bibliography|works cited|literature cited)\s*#*\s*$',
        re.IGNORECASE,
    )
...
def _mineru_api_tokens():
    tokens = [
        (getattr(settings, 'MINERU_API_TOKEN', '') or os.getenv('MINERU_API_TOKEN', '')).strip(),
        (getattr(settings, 'MINERU_API_TOKEN_2', '') or os.getenv('MINERU_API_TOKEN_2', '')).strip(),
        (getattr(settings, 'MINERU_API_TOKEN_3', '') or os.getenv('MINERU_API_TOKEN_3', '')).strip(),
    ]
    unique = []
    seen = set()
    for token in tokens:
        ...
        unique.append(token)
    if not unique:
        raise MinerUError('No MinerU API token configured. Set MINERU_API_TOKEN (and optional _2/_3).')
    return unique


def _mineru_json_headers(api_token=None):
    token = (api_token or '').strip()
    if not token:
        tokens = _mineru_api_tokens()
        token = tokens[0]
